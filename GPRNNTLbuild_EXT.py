...
"""print("TabPFN classifier training in progress...")
...
for train_i, val_i in skf.split(X_scaled, (Y_cls > 0.5).astype(int)):

    X_tr, X_val = X_scaled[train_i], X_scaled[val_i]
    Y_tr, Y_val = Y_cls[train_i], Y_cls[val_i]

    tabpfn_cls = TabPFNClassifier()
    tabpfn_cls.fit(X_tr, Y_tr)

    split_probs = tabpfn_cls.predict_proba(X_val)[:, 1]
    split_pred = tabpfn_cls.predict(X_val)

    acc_TabPFN.append(split_pred.flatten() == Y_val.flatten())
    f1_TabFPFN.append(f1_score(Y_val, split_pred))
    roc_TabFPFN.append(roc_auc_score(Y_val, split_probs))

print("------Classification TabPFN results----------")
print(f"TL Split Accuracy: {np.mean(acc_TabPFN):.4f}")
print(f"TL Split F1: {np.mean(f1_TabFPFN):.4f}")
print(f"TL Split ROC-AUC: {np.mean(roc_TabFPFN):.4f}")

print("Training TabPFN classifier...")

tabpfn_cls = TabPFNClassifier()

tabpfn_cls.fit(X_scaled, Y_cls)

split_probs = tabpfn_cls.predict_proba(X_scaled)[:, 1]
split_pred = tabpfn_cls.predict(X_scaled)
...
print("TabPFN regressor training in progress...")
...
for train_i, val_i in kf.split(X_scaled):
    ...
    X_tr, X_val = X_scaled[train_i], X_scaled[val_i]
    Y_tr, Y_val = Y[train_i], Y[val_i]   # regression target

    tabpfn_reg = TabPFNRegressor()
    tabpfn_reg.fit(X_tr, Y_tr)

    tabpfn_pred = tabpfn_reg.predict(X_val)

    rmse_TabPFN.append(np.sqrt(mean_squared_error(Y_val, tabpfn_pred)))
    mae_TabPFN.append(mean_absolute_error(Y_val, tabpfn_pred))
    r2_TabPFN.append(r2_score(Y_val, tabpfn_pred))

print("------Regression TabPFN results----------")
print(f"CV RMSE: {np.mean(rmse_TabPFN):.4f}")
print(f"CV MAE:  {np.mean(mae_TabPFN):.4f}")
print(f"CV R²:   {np.mean(r2_TabPFN):.4f}")

print("Training TabPFN regressor on full dataset...")

tabpfn_reg = TabPFNRegressor()
tabpfn_reg.fit(X_scaled, Y)

split_pred = tabpfn_reg.predict(X_scaled)

rmse = np.sqrt(mean_squared_error(Y, split_pred))
mae = mean_absolute_error(Y, split_pred)
r2 = r2_score(Y, split_pred)

print(f"RMSE (full data): {rmse:.4f}")
print(f"MAE (full data):  {mae:.4f}")
print(f"R² (full data):   {r2:.4f}")"""